...
def compare_models(prompt: str) -> dict:
    ...
    # Ước lượng chi phí GPT-4o (output)
    gpt4o_cost_estimate = (
...
def chat_with_system_prompt(
    ...
    """
    Gọi API với MESSAGES gồm system prompt và user prompt.
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    Nếu không thể dùng tiktoken (offline, model chưa hỗ trợ, ...),
    dùng ước lượng: 1 token ≈ 4 ký tự.
    """
    try:
        import tiktoken
        ...
        try:
            ...
            # Model chưa được tiktoken hỗ trợ
            enc = tiktoken.get_encoding("cl100k_base")

        return len(enc.encode(text))

    except Exception:
        # Fallback khi không có mạng hoặc chưa cài tiktoken
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Chỉ giữ 3 lượt hội thoại gần nhất (6 messages)
        history = history[-6:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        rows.append(
            f"{shorten(r['prompt']):<40} | "
            f"{shorten(r['gpt4o_response']):<40} | "
            f"{shorten(r['mini_response']):<40} | "
            f"{r['gpt4o_latency']:.3f}s".rjust(15) + " | "
            f"{r['mini_latency']:.3f}s".rjust(13)
        )
